...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        KASSERT(file);//PRE
        KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));//PRE
        dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
        KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);//MIDDLE
        dbg(DBG_PRINT, "(GRADING2A 1.a)\n");
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);//MIDDLE
        dbg(DBG_PRINT, "(GRADING2A 1.b)\n");
        dbg(DBG_PRINT, "(GRADING2A)\n");

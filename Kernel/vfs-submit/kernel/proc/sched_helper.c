...
sched_wakeup_on(ktqueue_t *q)
{
        ...
        if (!sched_queue_empty(q)){//only dequeue if queue is occupied
...
sched_broadcast_on(ktqueue_t *q)
{
        ...
        while (!sched_queue_empty(q)){//iterate until queue is empty to broadcast on all threads
                kthread_t *thr = sched_wakeup_on(q);

...
sched_cancellable_sleep_on(ktqueue_t *q)
{
        ...
        if (curthr->kt_cancelled == 1){//only if cancelled
...
sched_cancel(struct kthread *kthr)
{
        ...
        kthr->kt_cancelled = 1;//performed regardless of state
        if (kthr->kt_state == KT_SLEEP_CANCELLABLE){//only if cancellable
...
sched_switch(void)
{
        ...
        while (sched_queue_empty(&kt_runq)) {//wait for runq to be occupied
                intr_disable();
                intr_setipl(IPL_LOW);
                intr_wait();
                intr_setipl(IPL_HIGH);
